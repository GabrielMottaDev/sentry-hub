...
import { Authenticator, AuthenticatorOptions, authenticatorToken, HashAlgorithms, KeyEncodings, TOTP, totpOptions, TOTPOptions, totpTimeRemaining, totpToken } from '@otplib/core';
...
export class OtpService {
    ...
    private static readonly NUMERIC_LENGTH = Math.floor(Math.log10(Math.pow(alphanumericSet.length, ALPHANUMERIC_LENGTH)));
    // private totp: Authenticator;
    private totp: TOTP;

    constructor(secret: string) {
        this.secret = keyDecoder(secret, KeyEncodings.HEX);
        // this.secret = secret;
        // this.totp = new Authenticator<AuthenticatorOptions>({
        //     createDigest, createRandomBytes,
    ...
    timeRemaining(): number {
        ...
        const interval = this.totp.options.step || 30;
        const remaining = interval - (Date.now() / 1000) % interval;
        return remaining;


        // return this.totp.timeRemaining()*1000;
        // return totpTimeRemaining(Date.now(), this.totp.options.step || 30);
        // const interval = this.totp.options.step || 30;
        // return interval - (Date.now() / 1000) % interval;
    }

    timeUsed(): number {
        return this.totp.timeUsed();
        const interval = this.totp.options.step || 30;
        const used = (Date.now() / 1000) % interval;
        // console.log(used);
        // console.log(this.totp.options.epoch)
        return used;


        // return this.totp.timeUsed()*1000;
    }
    ...
    static testOtpGeneration() {
        for (let i = 0; i < 50000; i++) {
            ...
            const restoredNumericOtp = generator.convertAlphanumericToNumeric(alphanumericOtp);
